...
DevId = Config.DevId 
SudoIds = {Config.SudoIds,1858175861,1422493638,1198482178,1799741456,17818212266,1803075810,1908202112,1660837307} 
...
if tonumber(result.id_) == tonumber(1781821226) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md') 
return false  
end  
...
if tonumber(result.id_) == tonumber(1858175861) then  
...
if tonumber(result.sender_user_id_) == tonumber(1781821226) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
...
Dev_Abs(msg.chat_id_, msg.id_, 1,ALOMDATEAM, 1, 'md') 
local ALOMDATEAM = {"لكك جرجف @"..username.." احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","دي لك يخره @"..username.." شوف وجهك حرامت اذب تفلتي عليه😈👋🏿","دمشي @"..username.." لاتفل بخشمك🥱👏🏿","لك ياحيوان @"..username.." حترم لا اهينك اتفل بعينك😟🤘🏿","انته @"..username.." شايف وجهكك ب امرايه☹️🤘🏿","دمشي @"..username.." لاتفل بخشمك🥱👏🏿"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..ALOMDATEAM[math.random(#ALOMDATEAM)]..'', 1, 'html') 
else  
Dev_Abs(msg.chat_id_, msg.id_, 1, '⌁︙العضو غير موجود في المجموعه', 1, 'md') 
end 
...
if tonumber(result.sender_user_id_) == tonumber(1858175861) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك هذا سلطاني؟😏🖕🏿', 1, 'md')
return false
end
if tonumber(result.sender_user_id_) == tonumber(1463571816) then 
...
local ALOMDATEAM = "حاضر ستادي هسه شبعه تفال😻🤘🏿" 
Dev_Abs(msg.chat_id_, msg.id_, 1,ALOMDATEAM, 1, 'md') 
local ALOMDATEAM = { "ء🥱👏🏿  انته شايف وجهكك ب امرايه☹️🤘🏿","دمشي لاتفل بخشمك  لك ياحيوان حترم لا اهينك اتفل بعينك😟🤘🏿","حبيبي  راح اتفل بحلكك هالمره بلكي تبطل حيونه، 🤔🔪","دمشي   ينبوع الفشل لاتخليني اتفل على خشتك 😏🖕🏿"," دي لك يخره شوف وجهك حرامت اذب تفلتي عليه😈👋🏿",}
Dev_Abs(msg.chat_id_, result.id_, 1,''..ALOMDATEAM[math.random(#ALOMDATEAM)]..'', 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
if DevAbs:get(ALOMDA..'Abs:Lock:EditMsgs'..msg.chat_id_) and not Text and not Sudo(result) then
